...
postStore.fetchPost((route.params.id)).then(res => {
  const { title, short_desc, content, tags } = res.data.data
...
const onSubmit = async () => {
  ...
  if (valid) {
    loading.value = true;
    postStore.updateById(route.params.id, toRaw(postData)).then((res) => {
      if (res.status == 200) {
        ...
        <VCardText class="d-flex flex-wrap justify-space-between flex-column flex-sm-row">
          <VForm ref="form" lazy-validation>
            <VRow>
              ...
              <VCol cols="12">
                <VFileInput v-model="postData.thumbnail" placeholder="Ảnh thumbnail" label="Ảnh thumnail" prepend-icon="tabler-camera">
