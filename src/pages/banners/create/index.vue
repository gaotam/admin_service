...
const onSubmit = async () => {
  ...
  if (valid) {
    loading.value = true
    bannerStore.createBanner(bannerData).then((res) => {
      if (res.status == 201) {
        isSnackbarVisible.value = true;
        bannerData.content = `<h2>Mua ngay</h2><p>Giá chỉ từ <span class="ql-size-large">5.000.000 VND</span></p>`,
        form.value.reset()
      }
    }).catch(err => {
      console.log(err);
            ...
            <VRow>
              <VCol cols="12">
                <VSelect v-model="bannerData.page" :items="pages" prepend-icon="tabler-app-window"
                  item-title="title" item-value="value" label="Chọn trang" return-object single-line />
              </VCol>
              <VCol cols="12">
                <VSelect v-model="bannerData.position" :items="positions" prepend-icon="tabler-layout-align-center"
                  item-title="title" item-value="value" label="Chon vị trí" return-object single-line />
              </VCol>
              <VCol cols="12">
                <VSelect v-model="bannerData.type" :items="types" prepend-icon="tabler-ad-filled"
                  item-title="title" item-value="value" label="Chọn kiểu" return-object single-line />
              </VCol>
              ...
              <VCol cols="12">
                <VFileInput v-model="bannerData.image" prepend-icon="tabler-camera" :rules="[requiredValidator]"
